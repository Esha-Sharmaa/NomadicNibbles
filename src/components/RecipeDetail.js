...
const RecipeDetail = () => {
  ...
  return !recipe ? (
    <SkeletonUI />
      ...
      <>
             <Link to="/" className="inline-block  text-black font-semibold text-sm  py-1 px-2 rounded border-b-2 border-b-black mt-2">
        Back to Home
      </Link>
    <div className="block md:flex gap-10 justify-center my-12 min-h-[100vh] w-10/12 md:w-7/12 m-auto">
      <div className="w-6/12 md:4/12 mx-auto mb-4">
        <img src={recipe?.image} alt="recipe"className="w-[100%] rounded-lg" />
      </div>
      <div className="w-12/12 md:w-8/12">
        <h1 className="font-roboto text-2xl font-medium"> {recipe.title}</h1>
        <div className="flex gap-6 my-6">
          <button
            className="bg-gray-800 text-white p-2 rounded font-roboto "
            onClick={() => toggleView()}
          >
